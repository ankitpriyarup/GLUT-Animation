...
struct Color { float R, G, B; };		//Color structure
struct Point { int x, y; };				//2D Point structure
typedef struct { int x, y; bool itouch, otouch; float angle, radius; Color color; } Dot;		//Represent a dot on screen

/* VARIABLES */
int SCREEN_WIDTH = 640, SCREEN_HEIGHT = 480;
int BACKGROUND_SIZE = 10;
Point clockOrigin = { 1000, 750 };		//Center position of scene 1 clock
int clockRadius = 400;					//Radius of scene 1 clock
//All the background colors which are interpolated in scene 1
Color backgrounds[] =
{
	Color{0.596f, 0.862f, 0.823f},
	Color{0.545f, 0.757f, 0.827f},
	Color{0.447f, 0.549f, 0.631f},
	Color{0.941f, 0.824f, 0.588f},
	Color{0.949f, 0.890f, 0.784f},
	Color{0.678f, 0.831f, 0.780f},
	Color{0.572f, 0.462f, 0.537f},
	Color{0.824f, 0.654f, 0.494f},
	Color{0.560f, 0.792f, 0.753f},
	Color{0.953f, 0.894f, 0.788f}
};
int curBackground = 0;					//Temporary variable to store current background from the above array
int hourRadius = 200, minuteRadius = 250;	//Radius of hour and minute hand of the clock
float dRadius = 6.0f;						//Dial radius of the clock
...
void initRendering()
{
	//Initialize rendering setting dept, orthographic projection
...
void display(void)
{
	...
	if (timer < 1000)
	{
		...
		if (timer > 800)
		{
			...
			renderBitmapString(170, 400, (void *)font, "Yesterday is gone. Tomorrow has not yet come.");
			glPopMatrix();
		...
		if (curR > backgrounds[(curBackground + 1)].R) curR -= 0.001f;
		if (curR < backgrounds[(curBackground + 1)].R) curR += 0.001f;
		if (curG > backgrounds[(curBackground + 1)].G) curG -= 0.001f;
		if (curG < backgrounds[(curBackground + 1)].G) curG += 0.001f;
		if (curB > backgrounds[(curBackground + 1)].B) curB -= 0.001f;
		if (curB < backgrounds[(curBackground + 1)].B) curB += 0.001f;

		/* If current color matches from the background color list increment the current background */
		if (abs(curR - backgrounds[(curBackground + 1)].R) < 0.01f && abs(curG - backgrounds[(curBackground + 1)].G) < 0.01f
			&& abs(curB - backgrounds[(curBackground + 1)].B) < 0.01f)
		{
